...
class HeartAddView(View):
    ...
    def post(self, request):
        ...
        # 添加收藏夹记录
        conn = get_redis_connection('default')
        heart_key = 'heart_%d'%user.id

        # 设置hash中sku_id对应的值
        # hset->如果sku_id已经存在，更新数据， 如果sku_id不存在，添加数据
        try:
            conn.hset(heart_key, sku_id,1)#在此出错
        except Exception as e:
            print(e)
...
class HeartInfoView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        heart_key = 'heart_%d'%user.id
        # {'商品id':商品数量, ...}
        heart_dict = conn.hgetall(heart_key)

        skus = []
        # 遍历获取商品的信息
        for sku_id,count in heart_dict.items():
            # 根据商品的id获取商品的信息
            sku = GoodsSKU.objects.get(id=sku_id)
            skus.append(sku)
        ...
        # 使用模板
        return render(request, 'my-heart.html', context)
